...
def reset_IP():
    with lock:
        ...
        if ip:
            ...
            # mark reset time to ignore packets captured before this instant
            reset_times[ip] = now

            # full clean reset of this IP state
            graylist.pop(ip, None)
            ip_stats.pop(ip, None)
            watchlist.pop(ip, None)

            # cleanup old reset markers
            to_remove = [
                k for k, t in reset_times.items()
                if now - t > HOSTS_WINDOW_SECONDS
            ]
            for k in to_remove:
                del reset_times[k]
